...
def load_project(directory):
    ...
    if (
        (conda_path and pip_path) or
        (conda_path and docker_env) or
        (docker_env and pip_path) or
        (docker_env and pip_path and conda_path)
    ):
        raise ExecutionException(
    ...
    return Project(conda_env_path=conda_env_path, entry_points=entry_points,
                   docker_env=docker_env, pip_path=pip_env_path, name=project_name)
...
def _get_path_from_spec(spec, directory, default_file_name):
    if spec:
        ...
        if not os.path.exists(env_path):
            raise ExecutionException("Project specified environment file %s, but no such "
                                     "file was found." % env_path)
        return env_path
