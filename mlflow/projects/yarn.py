...
def run_yarn_job(remote_run, uri, entry_point_obj, final_params, extra_params,
                 experiment_id, backend_config=None):
    ...
    _logger.info("run = %s , uri = %s, command = %s , experiment_id = %s" % (remote_run, uri, command, experiment_id))
...
def _submit(skein_client, module_name, args=None, name="yarn_launcher",
           num_cores=1, memory="1 GiB", archive_hdfs=None,
           hadoop_file_systems=None, queue=None, env_vars=None,
    ...
    service = skein.Service(
        ...
        script=f'''
                    set -x
                    env
                    export HADOOP_CONF_DIR={env["HADOOP_CONF_DIR"]}
...
# ret = dict(value.split('=') for value in values)
